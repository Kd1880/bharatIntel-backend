...
# ── APScheduler (runs pipeline inside the web process — no separate worker needed) ──
try:
...
# ─────────────────────────────────────────────
# EMBEDDED PIPELINE SCHEDULER
# Runs inside the web process — no separate Render worker needed.
# ─────────────────────────────────────────────
def _run_pipeline_safe():
    try:
        ...
        log.error(f"Embedded scheduler pipeline error: {e}")

@asynccontextmanager
async def lifespan(app: FastAPI):
    if _SCHEDULER_AVAILABLE and os.getenv("ENABLE_SCHEDULER", "true").lower() == "true":
        sched = BackgroundScheduler()
        interval_min = int(os.getenv("PIPELINE_INTERVAL_MINUTES", "30"))
        sched.add_job(_run_pipeline_safe, "interval", minutes=interval_min, id="pipeline")
        sched.start()
        log.info(f"[Scheduler] Pipeline will run every {interval_min} min")
        import threading
        threading.Timer(10, _run_pipeline_safe).start()
    else:
        log.warning("[Scheduler] Disabled — apscheduler missing or ENABLE_SCHEDULER=false")
    yield
